...
algorithm.
In fact, this problem is particularly well-suited to being computed with the VOF interface  
tracking algorithm, since it consists of two distinct, immiscible fluids and interface tracking 
algorithms are specifically designed not to allow the two fluids to mix.
...
interface between the two fluids, one can use the output of the VOF method to examine the interface within individual cells or within regions consisting of groups of cells.
...
of the Rayleigh-Taylor\footnote{This is true whether the two fluids have the same viscosity or difference viscosities.} problem and hence, it is unstable at all wavelengths 
(e.g.~\cite{SC:1961} and therefore, it is very sensitive to the initial conditions.
...
$0$ and the other having the value of $1$~\cite{JMR:2019,JMR-EGP:2019} instead we compute the  
initial volume fractions using a level set function as follows.
First we compute a function, which is a \textit{signed distance level set function}; i.e. a 
function that is positive in the region that contains one of the fluids, which we will refer to as fluid~1 and negative elsewhere and the
...
In particular, the combination of \texttt{Number of initialization samples} with
