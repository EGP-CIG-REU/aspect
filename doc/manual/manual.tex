...
\paragraph{A Volume-of-Fluid Computation of the van Keken Problem}
...
\begin{figure}
    ...
    \caption{\it The result of the two computations of the Van Keken benchmark made with the 
      VOF interface tracking algorithm.
      The visualization here consists of \textit{two contours} for the reconstructed interface superimposed on red and blue colors, which are the two fluids reprefluid data (red and blue)for the
...
of integrating the initial interface over the cell with one material or fluid having the value 
of $1$ and the other having the value of $0$~\cite{JMR:2019,JMR-EGP:2019}, instead we compute 
the initial volume fractions using a signed distance function $\phi$ as follows.
...
This is accomplished by plotting the zero contour of the function $\phi$.
...
We have made a number of computations of the van Keken problem with the VOF method, which are intended to compare directly with the computational results produced using a compositional field to model the two fluids shown above.
In all of the VOF computations shown here we uses a CFL number of $\sigma=0.5$.

We begin by examining the RMS velocity data shown in Figure~\ref{fig:vof-vk-1},
and note that other than the case of 5 levels of global refinement the lines for
