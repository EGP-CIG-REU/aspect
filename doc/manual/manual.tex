...
However note that this implies that all computations of the van Keken problem with the VOF 
...
\includegraphics[width=0.5\textwidth]{cookbooks/benchmarks/vof-van-keken/rms_vel_comparison.png}
   \caption{Evolution of the root mean square velocity as a function of time with five 
    different global mesh refinements for computations of the Van Keken problem made with the 
    VOF interface tracking algorithm.
    Since the VOF initial conditions are discontinuous, the above results should   
    \textit{technically} be compared with the computations made with discontinuous initial 
    conditions on the left in Figure~\ref{fig:vk-2}, although the above results also compare 
    very favorably with the computations of the Van Keken problem made  with smoothed, 
    continuous initial conditions for the compositional field on the right in  
    Figure~\ref{fig:vk-2}.
...
\begin{figure}
    ...
    \caption{\it The result of two computations of the Van Keken problem made with the VOF 
...
of the Rayleigh-Taylor\footnote{This is true whether the two fluids have the same viscosity or 
different viscosities.} problem, it is unstable to perturbations of all wavelengths 
...
fluid has the value of $1$ and the other has the value of $0$~\cite{JMR:2019,JMR-EGP:2019},
instead we compute the initial volume fractions using a signed distance function $\phi$ as 
follows.
...
While the original visualization configuration in a typical parameter file is sufficient for
most purposes, when using the VOF method one has the ability to see the division between the 
fluids reconstructed by the VOF algorithm in each cell.
...
compare the VOF results directly with the those produced with a compositional field, which were 
presented in the previous section ~\ref{paragraph:van-keken compositional fields}.
...
velocities for $6, 7, 8, 9$ levels of refinement are nearly indistinguishable.

When examining the final solution, we note that the general structure of the solution shown in Figure~\ref{FIG:VOF_van_Keken-02} matches the expected form, and the
...
Average runtimes for global refinements 5 through 9 using one node can be seen
...
As such, we return to our examination of the initial condition and vary the size
...
parameter.
As such, our results show both the usefulness of the volume of fluid
