...
Furthermore, since one of the primary aspects of interest in the computation is the form of the 
interface between the two fluids, one can use the output of the VOF method to examine the 
interface within individual cells or within regions consisting of groups of cells.

We note that one modification of the computational parameters that is required in our VOF computation is that we use a CFL number of $\sigma=0.5$ in all of our computations.

Since, as noted above, when the interface is discontinuous, the van Keken problem is a version 
of the Rayleigh-Taylor\footnote{This is true whether the two fluids have the same viscosity or different viscosities.} problem and hence, it is unstable to all wavelengths 
(e.g.~\cite{SC:1961}).
Therefore, it is very sensitive to the initial conditions.
Thus, rather than use the standard initialization for the volume fractions, which is by 
integrating the initial interface over the cell with one material or fluid having the value of 
$1$ and the other having the value of $0$~\cite{JMR:2019,JMR-EGP:2019} instead we compute the  
initial volume fractions using a signed distance function $\phi$ as follows.
...
in the complement of this region, which we will refer to as fluid~2 and 2) at each point in the 
domain the magnitude $| \phi |$ of $\phi$  is the distance to the boundary between the 
...
possible on the coarser grid on which we will compute the time evolution of the interface. 
...
dividing each grid cell into $16 \times 16$ subcells and distance to the given initial 
...
than the standard initialization by quadrature, which we described above.
...
general use, when using the VOF method one has the ability to see the division between the 
fluids reconstructed by the VOF algorithm in each cell.

The modifications to the parameter file, which are necessary in order to draw the reconstructed 
boundary as a contour are in

\lstinputlisting[language=prmfile]{cookbooks/benchmarks/vof-van-keken/postprocess.part.prm.out}

(The full configuration file for this version of the benchmark problem can be found at \url{cookbooks/van-keken-vof.prm}).
