...
\begin{figure}
    ...
    \caption{\it The results of two computations of the Van Keken problem made with the VOF 
        ...
        $\tilde{\phi}^7 \, (t_{end})$ and $\tilde{\phi}^8 \, (t_{end})$, where 
        $\tilde{\phi}^7 \, (t_{end})$ is field where the zero countour is the reconstructed interface 
        at the final time $t_{end}$ computed on a uniform grid with a refinement level of 7 and 
        similarly for $\tilde{\phi}^8 \, (t_{end})$.
        The color of the zero contour for $\tilde{\phi}^7 \, (t_{end})$ is black and the color of the 
        zero contour $\tilde{\phi}^8 \, (t_{end})$ is bright green.
...
This is accomplished by plotting the zero contour of a field $\tilde\phi$ that
is generated to be $0$ at the reconstructed interface, positive in the region
with fluid~1, and negative in the region with fluid~2.
However $\tilde{\phi}$ does not satisfy the requirement that the maginitude is
equal to the distance to the interface as would be required for a signed
distance function.
...
\begin{table}
        ...
        \begin{tabular}{|c|c|c|c|}
            ...
            10 & 16 &         5.17 days\phantom{minutes}  & >6 days \\
            \hline
        \end{tabular}
        \caption{\it Comparison of average runtimes for the van Keken problem with VOF and and a DG computational field, in which the initial conditions for DG smoothed as described in section~\ref{paragraph:van-keken compositional fields} above.
            ...
            We note that the change in the CFL number $\sigma$ and the differing
            choice of cluster makes a direct quantitative comparison between this
            table and table~\ref{tab:runtime-table} invalid due to too many
            confounding factors.
        }
...
$10^{-3}\leq C \leq 1 - 10^{-3}$ band on the order of the same size as the amplitude 
variation shown here would be approximately $2.8 \cdot 10^{-4}$.
