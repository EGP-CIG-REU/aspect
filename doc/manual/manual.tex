...
first time step. As expected, we see that the location of the highest mesh refinement
...
\begin{figure}
   ...
   \caption{\it Mesh evolution during the first time step (refinement is based on strain rate).}
   \label{fig:meshes}
\end{figure}
...
in a neighborhood of the interface, rather in the entire computational domain.
For example, compare Tables~\ref{tab:runtime-table} and~\ref{tab:vof-runtime-comparison-table}.
...
\includegraphics[width=0.5\textwidth]{cookbooks/benchmarks/vof-van-keken/rms_vel_comparison.png}
   \caption{Evolution of the root mean square velocity as a function of time for computations 
    ...
    Figure~\ref{fig:vk-2}, although the above results also compare extremely favorably with the 
    computations with smoothed, continuous initial conditions for the compositional field on 
    the right in Figure~\ref{fig:vk-2}.
    As in Figure~\ref{fig:vk-2}, 5 global refinements correspond to a $32 \times 32$ mesh and 9 
    global refinements correspond to a $512 \times 512$ mesh.}
    \label{fig:vof-vk-1}
\end{figure}
...
\begin{figure}
    ...
    interface tracking algorithm  overlaid upon each other.
    This visualization consists of the \underline{two} zero contours of 
    $\hat{\phi}^7 \, (t_{end})$ and $\hat{\phi}^8 \, (t_{end})$, where 
    $\hat{\phi}^7 \, (t_{end})$ is the signed distance function to the reconstructed interface 
    at the final time $t_{end}$ computed on a uniform grid with a refinement level of 7 and 
    similarly for $\hat{\phi}^8 \, (t_{end})$.
    The color of the zero contour for $\hat{\phi}^7 \, (t_{end})$ is black and the color of the 
    zero contour $\hat{\phi}^8 \, (t_{end})$ is bright green.
    Compare with the fourth image on the right in Figure~\ref{fig:vk-1}.
    \textcolor{red}{Jon, What is the output time?}}
    \label{fig:VOF_van_Keken-02}
\end{figure}

Since, as noted above, when the interface is discontinuous, the van Keken problem is a version 
of the Rayleigh-Taylor problem,\footnote{This is true whether the two fluids have the same 
viscosity or different viscosities.} it is unstable to perturbations of all wavelengths 
(e.g.~\cite{SC:1961}), it is therefore, extremely sensitive to the initial conditions.
Thus, rather than use the standard initialization procedure for the VOF method, which consists 
of integrating the initial interface over each multi-material cell in which one material or 
fluid has the value of $1$ and the other has the value of $0$; i.e., a characteristic function 
representation of the interface~\cite{JMR:2019,JMR-EGP:2019}, instead we compute the initial volume fractions using a signed distance function $\phi$ as follows.
...
computations shown here of high-quality.
...
\lstinputlisting[language=prmfile]{cookbooks/benchmarks/vof-van-keken/main.part.prm.out}
...
% AMR section, currently unused so not included
% Due to the change in the advection algorithm, we also modify the refinement
% strategy for the AMR algorithm.
% In particular, the use of the \texttt{volume of fluid interface} refinement
% strategy will ensure that the boundary between that materials, and a small
% region around that will be refined to the maximum extent.\footnote{The
%     inclusion of this refinement strategy is currently required for use of the VOF
%     advection scheme in any model which makes use of AMR due to implementation
% details.}
% The new mesh refinement parameter section is shown below.
% 
% \lstinputlisting[language=prmfile]{cookbooks/benchmarks/vof-van-keken/amr.part.prm.out}
...
This is accomplished by plotting the zero contour of the computed signed distance function 
$\hat{\phi}$.
...
\lstinputlisting[language=prmfile]{cookbooks/benchmarks/vof-van-keken/postprocess.part.prm.out}
\noindent

We made a number of computations of the van Keken problem with the VOF method in order to 
compare the wall clock times of the VOF results directly with the those computed with 
compositional field, which are presented in the previous
section~\ref{paragraph:van-keken compositional fields}.
In all of the computations shown here we used a CFL number of $\sigma=0.5$.
...
velocities for $6$, $7$, $8$, and $9$ levels of refinement in Figure~\ref{fig:vof-vk-1} are 
...
are minor, although still slightly visible.
...
calculation, we ran both on the same cluster at varying refinement levels.
Average runtimes for global refinements 5 through 9 using one node with four CPUs are shown in 
Table~\ref{tab:vof-runtime-comparison-table}.
...
\begin{table}
        ...
        \begin{tabular}{|c|c|c|c|}
            ...
            \hline
            5 & 4 & \phantom{6}1.33 minutes\phantom{hours} & \phantom{-}2.57 minutes \\
            6 & 4 & \phantom{6}8.51 minutes\phantom{hours} & 19.5 minutes \\
            7 & 4 & \phantom{6}1.15 hours\phantom{minutes}   & \phantom{-}2.49 hours \\
            8 & 4 & \phantom{6}8.53 hours\phantom{minutes}   & 19.6 hours \\
            9 & 16 &          16.30 hours\phantom{minutes}  & 2.72 days \\
            \hline
        \end{tabular}
        \caption{\it Comparison of average runtimes for the van Keken problem with VOF and and a DG computational field, in which the initial conditions for DG smoothed as described in section~\ref{paragraph:van-keken compositional fields} above.
        The times shown are for the full computation, ending at $t_{end} = 2000$ with a CFL
        number of $\sigma=0.5$ in both cases. 
        All of these computations were made with ASPECT version 2.2.0-pre (master, ef542ecc2) in release mode on the Peloton2 cluster at U.C.~Davis.
            \textcolor{red}{Was their Table~\ref*{tab:runtime-table} made with DG or `regular' compositional fields.}}
        \label{tab:vof-runtime-comparison-table}
\end{table}

The consistency of the results shown here differs noticeably from the behavior of the same 
problem when computed with the FEM and DG advection algorithms.
We now study one of the possible reasons for these differences; namely, the procedure for 
defining the initial interface.
\textcolor{red}{Jon, This isn't true: ``given that both can be expected to converge from 
consistent initial conditions.'', since the van Keken problem is a version of the 
Rayleigh-Taylor problem, which is unstable. Read the first chapter or so of 
Chandrasekhar~\cite{SC:1961}. The RMS velocity may converge.  I am not sure if there is a 
proof of this one way or the other.}
As described above, we have designed an approach for setting the initial condition that should 
reproduce a smooth curve to a high degree of accuracy and, in particular, the curve, which is
where the discontinuity occurs in the discontinuous initial condition version of this problem.

In order to study this feature of our algorithm, we vary the size of the initial interface 
perturbation in order to examine the sensitivity of the final results to a small change in the 
initial conditions.
Specifically, we vary the amplitude $a$ of the cosine function in the initial conditions, as 
shown below.

\lstinputlisting[language=prmfile]{cookbooks/benchmarks/vof-van-keken/variation.part.prm.out}

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.5\textwidth]{cookbooks/benchmarks/vof-van-keken/init_diff_rms_vel_comparison.png}
    \caption{\it Computations of the Van Keken problem made with the VOF interface tracking 
    algorithm showing the evolution of the RMS velocity as a function of time for small changes in the amplitude $a$ of the cosine function in the initial condition.
    Compare to Figures~\ref{fig:vk-6} and~\ref{fig:vof-vk-3}.
    \textcolor{red}{Jon, Is this for 7 levels of refinement? Also, the legend should be `a = 0.019,' etc.}}
    \label{fig:vof-vk-3}
\end{figure}
...
the standard VOF initialization procedure described above unless the cell size is on the scale 
...
$\cdot10^{-3}\leq C \leq 1 - 10^{-3}$ band on the order of the same size as the amplitude 
variation shown here would be approximately $2.6 \cdot 10^{-4}$.
\textcolor{red}{Jon, I assume you worked the above order of magnitude calculations out on paper 
or in LaTeX. May I please see your calculations?  I am so busy right now I don't have time to 
do this myself. 
The reviewer(s) may also want to see the calculations.}
...
layers is introduced and is characterized by an amplitude $\Delta$ and a
