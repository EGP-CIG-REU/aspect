...
of the Rayleigh-Taylor problem it is unstable to perturbations of all 
wavelengths,\footnote{This is true whether the two fluids have the same viscosity or different 
viscosities.} (e.g. see~\cite{SC:1961}), it is therefore, extremely sensitive to the initial 
conditions.
...
compare the wall clock times of the VOF results directly with the those computed with a DG
compositional field and the results shown in Table~\ref{tab:runtime-table}.
In all of the computations shown here we used a CFL number of $\sigma=0.5$.
However, due to the change in the CFL number from $\sigma = 1.0$ in 
Table~\ref{tab:runtime-table} to $\sigma = 0.5$ in Table~\ref{tab:vof-runtime-comparison-table} 
and the difference between HPC clusters we can't make direct quantitative comparison between 
the data in Tables~\ref{tab:runtime-table} and~\ref{tab:vof-runtime-comparison-table}.
...
To compare the required time for the VOF computation to that for a DG computation, we ran both 
on the same cluster at varying refinement levels. 
Average runtimes for global refinements 5 through 10 using one node with four CPUs are shown in 
Table~\ref{tab:vof-runtime-comparison-table}.
We note that the use of the VOF advection algorithm significantly reduces the required computation time in all cases, frequently requiring less than half the time required by the DG computational field.
...
algorithms.
One of the possible reasons for these differences is the procedure for 
...
\begin{figure}[htb]
    ...
    \caption{\it Computations of the Van Keken problem made with the VOF
        ...
        Compare to Figures~\ref{fig:vk-6} and~\ref{fig:vof-vk-}.
    }
    \label{fig:vof-vk-3}
\end{figure}
