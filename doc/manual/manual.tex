...
\begin{figure}
    ...
    \caption{\it The results of two computations of the Van Keken problem made with the VOF 
        ...
        $\tilde{\psi}^7 \, (t_{end})$ and $\tilde{\psi}^8 \, (t_{end})$, where 
        $\tilde{\psi}^7 \, (t_{end})$ is field where the zero contour is the reconstructed interface 
        at the final time $t_{end}$ computed on a uniform grid with a refinement level of 7 and 
        similarly for $\tilde{\psi}^8 \, (t_{end})$.
        The color of the zero contour for $\tilde{\psi}^7 \, (t_{end})$ is black and the color of the 
        zero contour $\tilde{\psi}^8 \, (t_{end})$ is bright green.
...
compositional field.
We ran both on the same cluster at global refinements 5 through 10 using one node with four CPUs.
...
clusters on which the computational results shown in the two tables were made, we can't make 
...
\phantom{1}9 & 16           &         16.30 hours\phantom{minutes}  & \phantom{1}2.72 days\phantom{tes} \\
          10 & 16           & \phantom{1}5.17 days\phantom{minutess}&>6.00 days\phantom{tess} \\
            \hline
        \end{tabular}
        \caption{\it Comparison of runtimes for the van Keken problem with VOF and and a DG computational field, in which the initial conditions for DG smoothed as described in section~\ref{paragraph:van-keken compositional fields} above.
