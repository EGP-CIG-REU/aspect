...
{\it Possible values:} An integer $n$ such that $0\leq n \leq 2147483647$
\item {\it Parameter name:} {\tt Use VoF tracking}
\phantomsection\label{parameters:Use VoF tracking}


\index[prmindex]{Use VoF tracking}
\index[prmindexfull]{Use VoF tracking}
{\it Value:} true
...
{\it Description:} When set to true, VoF interface tracking will be used
...
{\it Value:} $ASPECT_SOURCE_DIR/tests/adiabatic-conditions/ascii-data/test/


{\it Default:} $ASPECT_SOURCE_DIR/tests/adiabatic-conditions/ascii-data/test/
...
{\it Value:} $ASPECT_SOURCE_DIR/data/boundary-traction/ascii-data/test/


{\it Default:} $ASPECT_SOURCE_DIR/data/boundary-traction/ascii-data/test/
...
{\it Value:} $ASPECT_SOURCE_DIR/data/geometry-model/initial-topography-model/ascii-data/test/


{\it Default:} $ASPECT_SOURCE_DIR/data/geometry-model/initial-topography-model/ascii-data/test/
...
{\it Value:} box_2d_%s.0.txt


{\it Default:} box_2d_%s.0.txt
...
{\it Value:} $ASPECT_SOURCE_DIR/data/adiabatic-conditions/ascii-data/test/


{\it Default:} $ASPECT_SOURCE_DIR/data/adiabatic-conditions/ascii-data/test/
...
{\it Value:} $ASPECT_SOURCE_DIR/data/initial-conditions/adiabatic-boundary/


{\it Default:} $ASPECT_SOURCE_DIR/data/initial-conditions/adiabatic-boundary/
...
{\it Value:} adiabatic_boundary.txt


{\it Default:} adiabatic_boundary.txt
...
{\it Value:} T1=1.5E3, L=1.0, D=1.0, pi=3.14159265359, Blsize=0.1, k=3, A=0.05, T_ref=0.75E3
...
{\it Value:} if(z<Blsize,T1*(1-z/D)+T1*A*sin(pi*z/D)*(1-cos(2*k*pi*x/L)),if(z>D-Blsize,T1*(1-z/D)-A*T1*sin(pi*z/D)*(1-cos(2*k*pi*x/L)),T_ref))
...
{\it Value:} $ASPECT_SOURCE_DIR/data/adiabatic-conditions/ascii-data/


{\it Default:} $ASPECT_SOURCE_DIR/data/adiabatic-conditions/ascii-data/
...
`vof interface': A class that implements a mesh refinement criterion, which ensures a minimum level of refinement near the VoF interface boundary.


{\it Possible values:} A comma-separated list of any of artificial viscosity, boundary, composition, composition gradient, composition threshold, density, maximum refinement function, minimum refinement function, nonadiabatic temperature, particle density, slope, strain rate, temperature, thermal energy density, topography, velocity, viscosity, vof interface
...
{\it Value:} top, bottom, left, right
...
{\it Value:} basic statistics, heat flux statistics, velocity statistics, visualization
...
{\it Possible values:} Any one of none, dx, ucd, gnuplot, povray, eps, gmv, tecplot, tecplot_binary, vtk, vtu, hdf5, svg, deal.II intermediate, txt
...
{\it Value:} $ASPECT_SOURCE_DIR/data/particle/generator/ascii/


{\it Default:} $ASPECT_SOURCE_DIR/data/particle/generator/ascii/
...
{\it Value:} artificial viscosity, density, viscosity, partition, vof values
...
`vof values': A visualization output object that outputs the  vof data.Names are given in Postprocess/Visualization/VoF values


{\it Possible values:} A comma-separated list of any of Vp anomaly, Vs anomaly, adiabat, artificial viscosity, artificial viscosity composition, boundary indicators, compositional vector, density, depth, dynamic topography, error indicator, friction heating, gravity, heat flux map, heating, material properties, maximum horizontal compressive stress, melt fraction, melt material properties, nonadiabatic pressure, nonadiabatic temperature, particle count, partition, seismic vp, seismic vs, shear stress, specific heat, strain rate, stress, thermal conductivity, thermal diffusivity, thermal expansivity, vertical heat flux, viscosity, vof values
...
{\it Possible values:} Any one of none, dx, ucd, gnuplot, povray, eps, gmv, tecplot, tecplot_binary, vtk, vtu, hdf5, svg, deal.II intermediate
...
\subsection{Parameters in section \tt Postprocess/Visualization/VoF values}
\label{parameters:Postprocess/Visualization/VoF_20values}

\begin{itemize}
\item {\it Parameter name:} {\tt Include internal reconstruction LS}
\phantomsection\label{parameters:Postprocess/Visualization/VoF values/Include internal reconstruction LS}


\index[prmindex]{Include internal reconstruction LS}
\index[prmindexfull]{Postprocess!Visualization!VoF values!Include internal reconstruction LS}
{\it Value:} true
...
{\it Description:} Include the internal level set data use to save reconstructed interfaces


{\it Possible values:} A boolean value (true or false)
\item {\it Parameter name:} {\tt Include normals}
\phantomsection\label{parameters:Postprocess/Visualization/VoF values/Include normals}


\index[prmindex]{Include normals}
\index[prmindexfull]{Postprocess!Visualization!VoF values!Include normals}
{\it Value:} false
...
{\it Description:} Include internal normal data in output (DEBUG)
...
{\it Value:} $ASPECT_SOURCE_DIR/data/prescribed-stokes-solution/


{\it Default:} $ASPECT_SOURCE_DIR/data/prescribed-stokes-solution/
...
\subsection{Parameters in section \tt VoF config}
\label{parameters:VoF_20config}

\begin{itemize}
\item {\it Parameter name:} {\tt Number of fields}
\phantomsection\label{parameters:VoF config/Number of fields}


\index[prmindex]{Number of fields}
\index[prmindexfull]{VoF config!Number of fields}
{\it Value:} 1
...
{\it Description:} The number of fields to be handled usingg VoF interface tracking.


{\it Possible values:} An integer $n$ such that $0\leq n \leq 2147483647$
\item {\it Parameter name:} {\tt Small volume}
\phantomsection\label{parameters:VoF config/Small volume}


\index[prmindex]{Small volume}
\index[prmindexfull]{VoF config!Small volume}
{\it Value:} 1e-14
...
{\it Description:} Minimum significant volume. VOFs below this considered to be zero.


{\it Possible values:} A floating point number $v$ such that $0 \leq v \leq 1$
\item {\it Parameter name:} {\tt VoF composition variable}
\phantomsection\label{parameters:VoF config/VoF composition variable}


\index[prmindex]{VoF composition variable}
\index[prmindexfull]{VoF config!VoF composition variable}
{\it Value:} VoFC
...
{\it Description:} Name of compositional field to write VoF composition to.


{\it Possible values:} A list of 0 to 4294967295 elements where each element is [Any string]
\item {\it Parameter name:} {\tt VoF field names}
\phantomsection\label{parameters:VoF config/VoF field names}


\index[prmindex]{VoF field names}
\index[prmindexfull]{VoF config!VoF field names}
{\it Value:} 
...
{\it Description:} User-defined names for VoF fields.


{\it Possible values:} A list of 0 to 4294967295 elements where each element is [Any string]
\item {\it Parameter name:} {\tt VoF solver tolerance}
\phantomsection\label{parameters:VoF config/VoF solver tolerance}


\index[prmindex]{VoF solver tolerance}
\index[prmindexfull]{VoF config!VoF solver tolerance}
{\it Value:} 1e-14
...
{\it Description:} The relative tolerance up to which the linear system for the VoF system gets solved. See 'linear solver tolerance' for more details.


{\it Possible values:} A floating point number $v$ such that $0 \leq v \leq 1$
\end{itemize}

\subsection{Parameters in section \tt VoF initial conditions}
\label{parameters:VoF_20initial_20conditions}

\begin{itemize}
\item {\it Parameter name:} {\tt Model name}
\phantomsection\label{parameters:VoF initial conditions/Model name}


\index[prmindex]{Model name}
\index[prmindexfull]{VoF initial conditions!Model name}
{\it Value:} function
...
\subsection{Parameters in section \tt VoF initial conditions/Function}
\label{parameters:VoF_20initial_20conditions/Function}

\begin{itemize}
\item {\it Parameter name:} {\tt Function constants}
\phantomsection\label{parameters:VoF initial conditions/Function/Function constants}


\index[prmindex]{Function constants}
\index[prmindexfull]{VoF initial conditions!Function!Function constants}
{\it Value:} d=1.0, k=0, a=0.01, L=6e6
...
{\it Possible values:} Any string
\item {\it Parameter name:} {\tt Function expression}
\phantomsection\label{parameters:VoF initial conditions/Function/Function expression}


\index[prmindex]{Function expression}
\index[prmindexfull]{VoF initial conditions!Function!Function expression}
{\it Value:} (0.5+1/8)*d-y
...
\phantomsection\label{parameters:VoF initial conditions/Function/Number initialization samples}


\index[prmindex]{Number initialization samples}
\index[prmindexfull]{VoF initial conditions!Function!Number initialization samples}
{\it Value:} 16
...
{\it Description:} Number of sampled points per dimension when initializing from VOF


{\it Possible values:} An integer $n$ such that $1\leq n \leq 2147483647$
\item {\it Parameter name:} {\tt Signed distance init}
\phantomsection\label{parameters:VoF initial conditions/Function/Signed distance init}


\index[prmindex]{Signed distance init}
\index[prmindexfull]{VoF initial conditions!Function!Signed distance init}
{\it Value:} true
...
{\it Description:} When set to true, initialization will be assumed to be asigned distance level set function.


{\it Possible values:} A boolean value (true or false)
\item {\it Parameter name:} {\tt Variable names}
\phantomsection\label{parameters:VoF initial conditions/Function/Variable names}


\index[prmindex]{Variable names}
\index[prmindexfull]{VoF initial conditions!Function!Variable names}
{\it Value:} x,y,t
